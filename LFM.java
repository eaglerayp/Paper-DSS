...
public class LFM {
    ...
    public static void main(String[] args) throws Exception {
       ...
       /* int dm_reply_size=0;
        ...
        for (task atask : tasklist) {
            executors.submit(new userOperate(replydm, socialdm, writerdm, features, iteration, atask,
                                             post_writer_table));
        }
        executors.shutdown();
        try {
        ...
        for (task atask : tasklist) {
            ...
            rankall[i++] = rank;
            String output_bias = atask.getTestuser() + "," + atask.getCoveruser() + "," + atask.getRank() + "," +
    ...
    public static long[] findTopOfSimilarity(Factorization mf, long userid, int size, FastIDSet excludeids,
                                             int startindex) throws TasteException {
    ...
    static class userOperate implements Runnable {
        ...
        public userOperate(DataModel dm, DataModel socialdm, DataModel writerdm, int features, int iteration,
                           task atask, HashMap<Long, Long> postwriter) {
            this.dm = dm;
...
//                        coveruser, postwriter);
                Factorizer f = new CliMF(dm, socialdm, writerdm, features, iteration, testuser, coveruser, postwriter);
...
//                Factorizer f = new SVDPPFactorizer(dm,socialdm, writerdm, features, iteration,  testuser,coveruser,postwriter);
                ...
                try {
                    topItems_bias = findTopOfSimilarity(mf, testuser, size, Excludeids, 0);
                } catch (Exception e) {
                    e.printStackTrace();
                }
...
//            String output=atask.getTestuser()+","+atask.getCoveruser()+","+atask.getRank()+","+atask.getInteractions()+","+atask.getTotalinteractions()+"\n";
    ...
    private static ArrayList<task> generate_task(HashSet<Long> SocialIDset, DataModel socialdm, DataModel dm,
                                                 HashMap<Long, Long> postwriter) throws
                                                                                 TasteException {
